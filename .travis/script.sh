...
then
	docker run -t -v "${PWD}:/libscca" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libscca && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libscca;
