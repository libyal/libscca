...
void (*sccatools_signal_signal_handler)( sccatools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI sccatools_signal_handler(
             sccatools_signal_t signal )
{
	static char *function = "sccatools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( sccatools_signal_signal_handler != NULL )
			{
				sccatools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void sccatools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int sccatools_signal_attach(
     void (*signal_handler)( sccatools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "sccatools_signal_attach";
	...
	sccatools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) sccatools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	sccatools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int sccatools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "sccatools_signal_detach";
	...
	sccatools_signal_signal_handler = NULL;
