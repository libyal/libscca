...
$ToolsTests = "info_handle output path_string signal"
$OptionSets = "" -split " "
...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$ResultRun = RunTestBinary ${TestExecutablesDirectory} "scca_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$ResultRun = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "sccainfo" ${TestProfile} "" ${TestInput}
