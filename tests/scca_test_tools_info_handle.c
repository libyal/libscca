...
#include "../sccatools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int scca_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int scca_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	SCCA_TEST_RUN(
	 ...
	 scca_test_tools_info_handle_initialize );

	SCCA_TEST_RUN(
	 "info_handle_free",
	 scca_test_tools_info_handle_free );
